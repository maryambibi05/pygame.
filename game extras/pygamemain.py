...
def orange_movement(keys, orange): # different movement keys for orange player. (d,g,r,f)
    if keys[pygame.K_a] and orange.x - velocity > 0: # moves to the left
        orange.x -= velocity
    if keys[pygame.K_d] and orange.x + velocity + orange.width < width: # moves to the right
        orange.x += velocity
    if keys[pygame.K_w] and orange.y - velocity > 0: # moves up
        orange.y -= velocity
    if keys[pygame.K_s] and orange.y + velocity + orange.height < height/2.3 + 10 : # moves down
        orange.y += velocity
        
def blue_movement(keys, blue):  # different movement keys for blue player. (h,k,u,j)
    if keys[pygame.K_LEFT] and blue.x - velocity > 0: # moves to the left
        blue.x -= velocity
    if keys[pygame.K_RIGHT] and blue.x + velocity + blue.width < width : # moves to the right
        blue.x += velocity
    if keys[pygame.K_UP] and blue.y + velocity > height/2: # moves up
        blue.y -= velocity
    if keys[pygame.K_DOWN]  and blue.y + velocity + blue.height < width/1.6 - 10: # moves down
        blue.y += velocity
